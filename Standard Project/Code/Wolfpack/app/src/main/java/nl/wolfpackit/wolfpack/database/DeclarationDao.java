...
public interface DeclarationDao{
    ...
    @Query("SELECT * FROM declaration WHERE name LIKE :search AND ( "+
                "EXISTS(SELECT * FROM declarationFeedback WHERE declaration.ID=declarationID AND not approved) " +
                "OR NOT EXISTS(SELECT * FROM declarationFeedback WHERE declaration.ID=declarationID)" +
            ")")
    List<Declaration> findBySearch(String search);

    @Query("SELECT * FROM declaration WHERE name LIKE :search AND EXISTS(SELECT * FROM declarationFeedback WHERE declaration.ID=declarationID AND approved)")
    List<Declaration> findAcceptedBySearch(String search);
